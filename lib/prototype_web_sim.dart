...
Future<void> sendChatMessage(String text, {TextEditingController? controller, XFile? attachedImage}) async {
  if (text.trim().isEmpty && attachedImage == null) return;
  ...
  try {
    if (attachedImage != null) {
      // 1. Upload image to OCR endpoint
      var request = http.MultipartRequest('POST', Uri.parse('http://127.0.0.1:8000/ocr-extract'));
      if (authService.isAuthenticated) request.headers['Authorization'] = 'Bearer ${authService.token}';
      
      if (kIsWeb) {
        final bytes = await attachedImage.readAsBytes();
        request.files.add(http.MultipartFile.fromBytes('file', bytes, filename: attachedImage.name));
      } else {
        request.files.add(await http.MultipartFile.fromPath('file', attachedImage.path));
      }
      
      var ocrResponse = await request.send();
      if (ocrResponse.statusCode == 200) {
        var responseData = await ocrResponse.stream.bytesToString();
        var ocrJson = jsonDecode(responseData);
        String ocrText = ocrJson['text'] ?? '';
        userMessage = "[Image OCR Scanned Text: \n$ocrText]\n\n$userMessage";
      }
    }
    ...
    final response = await http.post(
      ...
      headers: {
        ...
        if (authService.isAuthenticated) 'Authorization': 'Bearer ${authService.token}',
      },
    ...
    return InkWell(
      onTap: () {
        if (!isActive) {
          ...
          if (text == 'Fitness') screen = const FitnessScreen();
          else if (text == 'Meals') screen = const MealsScreen();
          else if (text == 'Profile') screen = const ProfileScreen();
          else if (text == 'AI Assistant') screen = const ChatScreen();
          else if (text == 'Web Inspector') screen = const WebInspectorScreen();
          Navigator.push(context, animatedRoute(screen));
        }
      },
  ...
  Widget _bottomNavItem(BuildContext context, IconData icon, String label, {bool isActive = false}) {
    return InkWell(
      onTap: () {
        if (!isActive) {
          Widget screen = const DashboardScreen();
          if (label == 'Fitness') screen = const FitnessScreen();
          else if (label == 'Meals') screen = const MealsScreen();
          else if (label == 'Profile') screen = const ProfileScreen();
          else if (label == 'AI Assistant') screen = const ChatScreen();
          else if (label == 'Web Inspector') screen = const WebInspectorScreen();
          Navigator.push(context, animatedRoute(screen));
        }
      },
      child: SizedBox(
...
class _AIWorkspaceState extends State<AIWorkspace> with SingleTickerProviderStateMixin {
  ...
  void _handleSend() {
    ...
    sendChatMessage(text, attachedImage: img);
  }
                        ...
                        if (_selectedImage != null)
                          Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                const Icon(Icons.image, size: 16, color: primaryGreen),
                                const SizedBox(width: 4),
                                Text(_selectedImage!.name, style: const TextStyle(fontSize: 12), overflow: TextOverflow.ellipsis),
                                InkWell(onTap: () => setState(()=>_selectedImage=null), child: const Icon(Icons.close, size: 16)),
                              ]
                            )
                          ),
                        TextField(
            ...
            if (isMobile(context))
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    IconButton(icon: const Icon(Icons.camera_alt_outlined, size: 20), onPressed: _pickImage),
    ...
    return Row(
      children: [
        IconButton(
          ...
          onPressed: () {
            Navigator.push(context, animatedRoute(const ChatScreen()));
          },
        ),
        IconButton(
          ...
          if (isDesktop(context))
            const Row(
              children: [
                ...
                SizedBox(width: 24),
                Expanded(child: _ActionCard(title: "Ingredient Scanner", desc: "Scan ingredient lists for harmful items and check dietary fit.", cta: "Scan Ingredients →", image: "assets/images/ingredient_safety_1787489740080.jpg")),
              ],
            )
          else if (isTablet(context))
            Column(
            ...
            Column(
              children: const [
                ...
                SizedBox(height: 16),
                _ActionCard(title: "Ingredient Scanner", desc: "Scan ingredient lists for harmful items and check dietary fit.", cta: "Scan Ingredients →", image: "assets/images/ingredient_safety_1787489740080.jpg"),
              ],
            )
        ],
      ),
    ...
    return GestureDetector(
      onTap: () {
        if (widget.title == 'Web Inspector') {
          Navigator.push(context, animatedRoute(const WebInspectorScreen()));
        } else if (widget.title == 'AI Assistant') {
          Navigator.push(context, animatedRoute(const ChatScreen()));
        } else if (widget.title == 'AI Diet Planner') {
          Navigator.push(context, animatedRoute(const DietPlannerScreen()));
        } else if (widget.title == 'Ingredient Scanner') {
      ...
      child: ContentContainer(
        child: Column(
          children: [
            ...
            TextButton(
              onPressed: () {
                Navigator.push(context, animatedRoute(const TaskScreen(title: 'Dashboard')));
              },
...
class WebInspectorScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Web Inspector', style: GoogleFonts.inter(color: primaryText, fontWeight: FontWeight.bold)),
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _handleSend() {
    ...
    sendChatMessage(text, controller: _controller, attachedImage: img);
  }
        ...
        children: [
          ...
          Container(
            ...
            child: SafeArea(
              child: Row(
                children: [
                  IconButton(icon: const Icon(Icons.camera_alt_outlined, color: Colors.grey), onPressed: _pickImage),
                  Expanded(
                    child: Container(
                      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                      decoration: BoxDecoration(
                            ...
                            Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  const Icon(Icons.image, size: 16, color: primaryGreen),
                                  const SizedBox(width: 4),
                                  Expanded(child: Text(_selectedImage!.name, style: const TextStyle(fontSize: 12), overflow: TextOverflow.ellipsis)),
                                  InkWell(onTap: () => setState(()=>_selectedImage=null), child: const Icon(Icons.close, size: 16)),
                                ]
                              )
                            ),
